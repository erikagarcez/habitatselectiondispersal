...
#-------------------------IMPORT OUTPUT DATA -----------------------------------
datao <- read.table(file = "~/Documentos/erika/doutorado/output/output.txt",sep = ",",header = T)
datap <- read.table(file = "~/Documentos/erika/doutorado/output/sim_parameters.txt",sep = ",",header = T)
data = merge(x = datao, y = datap, by = "run_number",
                all.x = TRUE)
# set directory to save figures
setwd("~/Documentos/erika/doutorado/output")
#-------------------------PACKAGES ---------------------------------------------
library(ggplot2)
library(cowplot)
library(dplyr)
...
g2 <- ggplot(dataq, aes(x=behaviour, y=meanQ, group=behaviour,colour=factor(behaviour))) +
  ...
  labs(y="", x = "Mean Habitat Quality of Settlement Area") + 
  guides(colour="none")
g2
...
g3 <- ggplot(dataen, aes(x=behaviour, y=meanen, group=behaviour,colour=factor(behaviour))) +
  ...
  theme(strip.background =element_blank(),
        ...
        #axis.text.y = element_blank(),axis.ticks.y=element_blank(),
        plot.margin = unit(c(0,.5,0,.5),"lines")) +
  labs(y = "",x="Mean Energetic Condition on Settlement",
...
png(file="general_mean_all.png",width=1500, height=2000,res = 150)
...
# EXPORT FIGURE
#------------------------------------------------------------------------------
png(file="density_plots.png",width=1500, height=1200,res = 150)
