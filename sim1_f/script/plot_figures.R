...
#-------------------------IMPORT OUTPUT DATA -----------------------------------
datao <- read.table(file = "~/Documentos/erika/doutorado/output/sim1_f/output.txt",sep = ",",header = T)
datap <- read.table(file = "~/Documentos/erika/doutorado/output/sim1_f/sim_parameters.txt",sep = ",",header = T)
data = merge(x = datao, y = datap, by = "run_number",
                all.x = TRUE)
# set directory to save figures
setwd("~/Documentos/erika/doutorado/output/sim1_f/figures/")
#-------------------------PACKAGES ---------------------------------------------
library(ggplot2)
library(cowplot)
library(dplyr)
...
data <- unique(data) # run 97 duplicated

set_table <- data %>% 
  group_by(run_number,behaviour,hab_amount,clumpiness,initial_individuals) %>% 
...
theme <- theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),panel.border = element_blank(),
...
g1 <- ggplot(set_table, aes(x=behaviour, y=rate, group=behaviour,colour=factor(behaviour))) +
  geom_boxplot() +
  geom_jitter(shape=1,size = 1,position=position_jitter(0.1),alpha=0.9) +
  scale_colour_brewer(palette = "Paired") +
  theme + ylim(0,1) +
  theme(axis.line = element_line(colour = "black"),
        axis.text.y = element_text(size = 8),
        plot.margin = unit(c(.5,-.5,.5,-0.5),"lines")) +
  labs(y="",x = "Settlement Rate") + 
  guides(colour="none")
g1

# Habitat quality in settlement
dataq <- data %>% 
  group_by(run_number,behaviour,clumpiness,hab_amount) %>% 
...
g2 <- ggplot(dataq, aes(x=behaviour, y=meanQ, group=behaviour,colour=factor(behaviour))) +
  ...
  theme(axis.line.x = element_line(colour="black"),
        ...
        plot.margin = unit(c(.5,0,.5,-1.2),"lines")) +
  labs(y="", x = "Mean Habitat Quality of \n Settlement Area") + 
  guides(colour="none")
g2
...
g3 <- ggplot(dataen, aes(x=behaviour, y=meanen, group=behaviour,colour=factor(behaviour))) +
  ...
  scale_colour_brewer(palette = "Paired") +
  theme + ylim(0,1) +
  theme(legend.position = c(0.1,0.9),legend.title=element_text(size=8),
        legend.direction = "horizontal",
        legend.text = element_text(size = 7)) +
  theme(axis.line.x = element_line(colour="black"),
        axis.text.y = element_blank(),axis.ticks.y=element_blank(),
        plot.margin = unit(c(.5,0,.5,-1.2),"lines")) +
  labs(y = "",x="Mean Energetic Condition \non Settlement",
       colour="Plasticity\nLevel") #+
...
g4 <- ggplot(datadt, aes(x=behaviour, y=meandt, group=behaviour,colour=factor(behaviour))) +
  ...
  labs(y="",x = "Mean Total Distance Moved (km)") + 
  guides(colour="none")
g4
...
g5 <- ggplot(datadl, aes(x=behaviour, y=meandl, group=behaviour,colour=factor(behaviour))) +
  ...
  theme(axis.line = element_line(colour = "black"),
        axis.text.y = element_text(size = 8),
        axis.title.x = element_text(size = 10),
        plot.margin = unit(c(.5,0.5,.5,0),"lines")) +
  labs(y="", x = "Mean Euclidian Distance Moved (km)",
       colour="Plasticity\nLevel") 
g5
...
png(file="general_mean_all.png",width=1000, height=1000,res = 150)
plot_grid(g1,g2,g3,
          axis = "blt",
...
png(file="general_mean_dist.png",width=900, height=800,res = 150)
plot_grid(g4,g5,
          axis = "blt",
          rel_widths = c(0.5,0.65),
          scale = c(1,1),
...
d1 <- ggplot(data, aes(x = Hab_quality_area, y = cat_land,colour=behaviour,fill=behaviour)) +
  ...
  labs(colour="Plasticity\nLevels",x="Habitat Quality in Settlement",y="Habitat Amount") +
...
# EXPORT FIGURE
#------------------------------------------------------------------------------
png(file="density_plots.png",width=1000, height=1200,res = 150)
...
#N_beh = 1000/5
data_land <- data %>% 
  group_by(run_number,behaviour,hab_amount,clumpiness) %>% 
  summarise(meanq = mean(Hab_quality_area),
...
strip.background = element_rect(colour = "black", fill = "gray95", size=0.3),
...
gg <- ggplot(data_land,aes(clumpiness,colour=hab_amount)) +
  ...
  facet_grid(~behaviour,labeller = labeller(behaviour=lab_beh)) +
...
# EXPORT FIGURE
#------------------------------------------------------------------------------
png(file="landscape__pointsplots.png",width=2800, height=2800,res = 250)
